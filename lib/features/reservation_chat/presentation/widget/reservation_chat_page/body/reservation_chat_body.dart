...
class _ReservationChatBodyState extends State<ReservationChatBody> {
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  return Padding(
                    ...
                    child: Wrap(
                        runAlignment: _adjustAlignment(chatMessages[index].sender),
                        crossAxisAlignment: _adjustCrossAlignment(chatMessages[index].sender),
                        direction: Axis.vertical,
                        children: [
                      Padding(
                        padding: EdgeInsets.only(left: padding/2, bottom: padding/2, right: padding/2),
                        child: Text(
                            _adjustSenderText(chatMessages[index].sender),
                            //textAlign: TextAlign.right,
                            style: Theme.of(context).textTheme.bodySmall
                        ),
                      ),
                      Tooltip(
                          message: chatMessages[index].sendTime.toString(),
                          child: CommonCard(
                              backgroundColor: chatMessages[index].sender == loggedUserId ? null : Color(0xFFFFFFF7),
                              child: Text(chatMessages[index].message, style: Theme.of(context).textTheme.bodyLarge),
                              onPressed: (){},
                          )
                      ),
                    ]),
  ...
  void _startScrollDownTimer(){
    if(scrollDownTimer == null || (scrollDownTimer!=null && !scrollDownTimer!.isActive)) {
      scrollDownTimer = Timer.periodic(const Duration(milliseconds: 50), (timer) {
  ...
  WrapCrossAlignment _adjustCrossAlignment(String sender){
    return sender == loggedUserId ? WrapCrossAlignment.end : WrapCrossAlignment.start;
  }
